...
export default function TeacherClassroomPage() {
    ...
    async (showLoadingState = false) => {
      try {
        ...
        if (prevCount > 0 && newCount > prevCount) {
          ...
          const newParticipantsList = data.participants.filter(
            (p: Participant) => !prevIds.has(p.id)
          );
          ...
          if (newParticipantsList.length > 0) {
            ...
            newParticipantsList.forEach((participant: Participant) => {
              showNotification(
                `${participant.displayName || 'Anonymous'} has joined the classroom.`,
                'success'
              );
            });
            ...
            setNewParticipantIds((prev) => [
              ...prev,
              ...newParticipantsList.map((p: Participant) => p.id),
            ]);
          }
        }
  ...
  const handleRemoveParticipant = async (participantId: number) => {
    try {
      ...
      if (participant) {
        showNotification(
          `${participant.displayName || 'Anonymous'} has been removed from the classroom.`,
          'info'
        );
      }
  ...
  // Handle game start and redirect all participants
  const handleStartGame = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/classroom/${id}/start-game`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      showNotification('Game started! All participants are being redirected.', 'success');

      // Close confirmation dialog
      setConfirmStartGame(false);

      // Ask user if they want to go to the game page as well
      if (confirm('Game started successfully! Do you want to go to the game page as well?')) {
        router.push('/game');
      }
    } catch (err: any) {
  ...
  return (
    <div className="container mx-auto py-10 px-4">
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between">
        ...
        <div className="mt-4 md:mt-0 flex items-start gap-2">
          <Button
            variant="default"
            size="sm"
            onClick={() => setConfirmStartGame(true)}
            disabled={participants.length === 0 || startingGame}
        ...
        <Dialog open={!!confirmDelete} onOpenChange={() => setConfirmDelete(null)}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Are you sure you want to remove this student from the classroom? They will be
                notified and can rejoin later if needed.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
      ...
      <Dialog open={confirmClassroomDelete} onOpenChange={setConfirmClassroomDelete}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to delete this classroom? This action cannot be undone. All
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmClassroomDelete(false)}>
              Cancel
            </Button>
            <Button variant="destructive" onClick={handleDeleteClassroom} disabled={refreshing}>
      ...
      <Dialog open={confirmStartGame} onOpenChange={setConfirmStartGame}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Start Game</DialogTitle>
            <DialogDescription>
              Are you ready to start the game? All participants will be redirected to the game page.
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
            <p className="text-sm mb-2">
              Current participants: <strong>{participants.length}</strong>
            </p>
            <p className="text-sm text-muted-foreground">
              When you start the game, all students will be redirected to the game page. You'll have
              the option to join them afterward.
            </p>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setConfirmStartGame(false)}>
              Cancel
            </Button>
            <Button
              variant="default"
...
function EmptyState() {
  ...
  return (
    <div className="container mx-auto py-20 px-4 text-center">
      ...
      <Button onClick={() => router.push('/teacher/classrooms')}>Return to Classrooms</Button>
    </div>
  );
}
