          ...
          {analysis.data?.map((item: ScoreBreakdown, index: number) => (
            ...
            >
              ...
              <p className="text-sm text-gray-600">
                Voortgang Score: {item.details.progressScore.toFixed(2)} | Doel
                Score: {item.details.targetScore.toFixed(2)}
              </p>
              <p className="text-sm text-gray-600">
                Gewogen Score: {item.details.weightedScore.toFixed(2)} | Domein
                Gewicht: {item.details.domainWeight.toFixed(2)}
              </p>
              <p className="text-sm text-gray-600">
                Absolute Verandering: {item.metadata.absoluteChange.toFixed(2)}{" "}
                | Percentage Verandering:{" "}
                {item.metadata.percentageChange.toFixed(2)}%
              </p>
            </div>
          ))}
        </div>
