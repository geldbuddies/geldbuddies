...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!classroomSession.isActive) {
      return NextResponse.json({ error: 'This classroom session is no longer active' }, { status: 400 });
    }
    ...
    if (classroomSession.maxPlayers) {
      const currentParticipantsCount = await db.query.classroomParticipants.findMany({
        where: and(
          eq(classroomParticipants.sessionId, classroomSession.id),
