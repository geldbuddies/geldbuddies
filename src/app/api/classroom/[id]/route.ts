...
export async function DELETE(req: NextRequest, context: { params: { id: string } }) {
  try {
    ...
    try {
      ...
      const deleteParticipantsResult = await db
        .delete(classroomParticipants)
    ...
    try {
      ...
      const deleteClassroomResult = await db
        .delete(classroomSessions)
    ...
    return NextResponse.json(
      {
        error:
          'Failed to delete classroom: ' + (error instanceof Error ? error.message : String(error)),
        requestUrl: req.url,
      },
      { status: 500 }
    );
